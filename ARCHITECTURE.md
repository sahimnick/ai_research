...
> The package is 62 modules / 21,249 lines / ~957 KB (~250k tokens). This file is ~8k tokens.
>
...
| Package layout | flat — all 62 modules directly under `neurobrain/` |
...
|---|---|---|---|---|
| `neuron` | 0 | 438 | Izhikevich neurons, SoA population, 7 cell types | `Population, Neuron, NeuronType, NEURON_TYPES, get_type, type_id` |
| `vision` | 0 | 1316 | Rate-coded ventral stream V1→V2→V4→IT, Gabors, stimuli | `VentralStream, SpikingConvLayer, ComplexCellLayer, ITLayer, build_ventral_stream, gabor_kernel, shape_images` |
| `sensors` | 0 | 238 | Encoders: text/image/sound/vector → currents | `TextEncoder, ImageEncoder, SoundEncoder, VectorEncoder` |
| `realworld` | 0 | 286 | MNIST / Fashion-MNIST download+cache, growing category map, OOD "don't know" | `load_mnist, load_fashion_mnist, GrowingCategoryMap, DigitRecognizer, contrast_normalise` |
...
| `objects` | 0 | 324 | Object-centric causal graph, rules learned *per property* | `CausalWorldGraph, ObjectConcept, Relation, build_causal_world` |
| `pallium` | 0 | 330 | Hierarchical recurrent settling + top-down prediction | `PredictivePallium, PalliumLayer, build_predictive_pallium` |
| `selfmodel` | 0 | 378 | Body limits, motor capability, interoception, agency | `SelfModel, BodyModel, AgencyModel, MotorCapability, Interoception` |
| `worldmodel` | 0 | 272 | Predictive transitions, successor representation, latent belief | `PredictiveWorldModel, GridWorld` |
| `world_model` | 0 | 213 | ⚠ *different module* — concept assemblies for `Brain` | `WorldModel, Concept` |
| `development` | 0 | 347 | Critical periods, staged growth, sleep replay | `DevelopmentalProgram, SleepConsolidator, EpisodicBuffer, Stage, sleep_benefit_experiment` |
| `analogy` | 0 | 112 | Relations between relations | `RelationalMind` |
| `reasoning` | 0 | 196 | Semantic network | `SemanticNetwork` |
| `relational` | 0 | 171 | Relational memory | `RelationalMemory` |
| `discovery` | 0 | 221 | Unsupervised concept discovery | `ConceptDiscovery, make_clustered_data` |
| `dendrite` | 0 | 155 | Dendritic trees, ~7000 syn/neuron, supralinear NMDA | `DendriticTree, build_cortical_column, dendritic_density_report` |
| `biophysical` | 0 | 542 | Continuous-time HH/AdEx circuit | `BiophysicalCircuit, nmda_mg_block` |
| `persistence` | 0 | 49 | gzip+pickle save/load ⚠ RCE risk | `save_brain, load_brain` |
| `synapse` | 1 | 457 | Projections, STDP, delays, tagging | `SynapseBundle, STDPConfig, TagConfig` |
| `region` | 1 | 156 | Functional area wrapper (sensory/assoc/motor/memory) | `Region, ROLE_*` |
| `audio` | 1 | 359 | Cochleagram, A1, belt, tone/chirp stimuli | `Cochleagram, AuditoryStream, build_auditory_stream, tone, chirp, sound_dataset` |
| `spikingvision` | 1 | 295 | Real spiking retina→features→category, honest cost | `SpikingRetina, SpikingDigitBrain, build_spiking_digit_brain` |
| `loop` | 1 | 400 | Closed predict/sense/correct loop + worlds | `CognitiveLoop, SensoryWorld, ContinuousWorld, LoopTrace, closed_loop_experiment` |
| `psyche` | 1 | 284 | Associative cortex + mental space, reconstruction | `ReconstructiveMind, MentalSpace, AssociativeCortex` |
| `grounding` | 1 | 324 | Visual features from causal prediction error only | `CausalFeatureLearner, GroundedCausalMind, grounded_vs_handtyped` |
| `dopamine` | 1 | 630 | RPE, eligibility traces, bandit / delayed reward | `DopaminergicActionLoop, RewardPredictionError, EligibilityTrace, BanditTask, compare_rules` |
| `backend` | 1 | 185 | Optional torch/CUDA/MPS population ⚠ breaks import | `TorchPopulation, best_device, available_devices, compare_backends` |
| `brain` | 2 | 424 | **The simulation environment**: regions + projections + world | `Brain, SpikeFrame` |
| `widev1` | 2 | 858 | Wide retinotopic spiking V1, 50 ms window, motion | `WideV1, WideDigitBrain, width_ablation, window_ablation, motion_experiment` |
| `assembly` | 2 | 333 | Spiking episodic memory, pattern completion | `SpikingEpisodicMemory, assembly_experiment` |
| `multimodal` | 2 | 238 | Vision+hearing association area | `AssociationArea, MultisensoryBrain, build_multisensory_brain` |
| `spikinghierarchy` | 2 | 304 | Spiking V1→V2→V3 + active causal discovery | `SpikingVentralHierarchy, active_causal_discovery` |
| `continuous` | 2 | 145 | Continuous predictor that beats the stay-baseline | `ContinuousPredictor, continuous_prediction_experiment` |
| `integrated` | 2 | 278 | Million-neuron multi-area substrate + sensory bridge | `IntegratedBrain, SensoryBridge, integration_experiment` |
| `tagging` | 2 | 241 | Synaptic tagging and capture | `synaptic_tagging_experiment` |
| `unified` | 2 | 385 | One mind: perceive+attend+comprehend+recall+dream | `UnifiedMind, build_unified_mind, pallium_capacity_curve` |
| `teacher` | 3 | 258 | Inductive imprinting lessons | `Teacher, Lesson` |
| `builder` | 3 | 287 | Factories for ready-to-teach brains | `build_default_brain, build_large_brain, build_comprehensive_substrate, BrainSpec` |
| `attention` | 3 | 109 | Region gains + top-down current | `Attention` |
| `imagination` | 3 | 125 | Offline replay current | `Imagination` |
| `mind` | 3 | 225 | Perception grounded into a world model | `Mind, build_mind` |
| `streams` | 3 | 929 | **Saccadic eye, continuous ear, scenes, soundscapes** | `Scene, SaccadicEye, Fixation, ContinuousEar, AuditoryBelt, Soundscape, StreamingBrain, streaming_experiment` |
| `rhythm` | 3 | 265 | Theta-gamma working memory capacity | `OscillatoryWorkingMemory, working_memory_experiment` |
| `endtoend` | 3 | 137 | Whole chain measured only at the far end | `EndToEndBrain, build_end_to_end` |
| `visualizer` | 3 | 288 | Matplotlib animation of the activation path | `NetworkVisualizer` |
| `curriculum` | 4 | 141 | First concepts taught | `teach_starter_curriculum, CurriculumReport` |
| `curriculum_advanced` | 4 | 206 | Harder lessons + classifier | `teach_advanced_curriculum, classify` |
| `knowledge` | 4 | 232 | Foundation knowledge brain | `teach_foundation, FoundationReport` |
| `language` | 4 | 145 | Word grounding | `LanguageModel` |
| `bigbrain` | 4 | 264 | Scale/capacity curves ⚠ Linux-only RSS | `CorticalMemory, run_cognition_on_substrate, substrate_capacity_curve` |
| `console` | 4 | 610 | **Live HTTP dashboard** of the integrated brain | `BrainConsole, serve_console` |
| `mapdebug` | 4 | 279 | Why did a SOM collapse? 5 diagnostics | `diagnose, MapDiagnosis, participation_ratio, winner_statistics` |
| `runtime` | 5 | 382 | Threaded live tick loop + homeostasis | `BrainRuntime` |
| `selforganize` | 5 | 537 | RFs discovered, not designed (vision + hearing) | `develop_v1, develop_a1, self_organized_vs_designed, orientation_selectivity` ⚠ |
| `auditorycortex` | 6 | 1232 | Contrastive-predictive A1/A2, gabor bank control | `PredictiveA1, ContrastivePredictiveA1, MultiScaleA1, linear_probe, predictive_a1_experiment` |
| `v2binding` | 6 | 363 | V2 units tuned to pairs of V1 features | `V2Binding, VentralV1V2, build_v1_v2, v1_parts` |
| `dashboard` | 6 | 495 | **ORPHAN** — superseded by `console.py` | (unreachable) |
| `composite` | 7 | 401 | Overlapping shapes that *require* binding | `LocalV2, composite_binding_experiment, draw_shape, part_overlap_report` |
| `perceptloop` | 7 | 401 | Full chain pixels→…→imagination + active perception | `PerceptualObjectMind, ObjectFile, full_loop_experiment` |
| `topdown` | 8 | 454 | Rao-Ballard prediction, staged growth, error-driven looking | `PredictiveStack, GrowthSchedule, ActiveLooker, predictive_coding_experiment` |
...
| **live browser dashboard** | `serve_console(port=8080)` or `python -m neurobrain.console` | blocking server | ~1 min startup |
...
| 22 | M | `world_model.py` vs `worldmodel.py` | Two different modules, near-identical names |
...
- **Question about spiking dynamics / neuron model** → `neuron.py` only.
- **Question about learning** → `synapse.py` (`apply_stdp`, `collect_current`). Nothing else learns by gradient.
- **Question about how two faculties communicate** → `workspace.py`. Always.
- **Question about "how does it see"** → `streams.py` (`SaccadicEye`) then `widev1.py`. `vision.py` is the *older rate-coded* stream; don't confuse them.
- **Question about "how does it hear"** → `streams.py` (`ContinuousEar`, `AuditoryBelt`) then `auditorycortex.py`. `audio.py` is the older/simpler path.
...
- **Question about running/demoing** → `console.py` (`serve_console`), not `dashboard.py`.
...
- **`vision.py` (1316 loc), `auditorycortex.py` (1232), `streams.py` (929), `widev1.py` (858)** are the only files over 800 lines. Everything else fits comfortably in context.
