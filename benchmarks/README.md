...
| `eye_loop.py` | Do the measured pieces actually compose into a running eye? | a demonstration **and a regression check**: every read-out is scored against the same control its own section used, so a system that differs from its measured components says so instead of hiding behind a working demo — which is how the size read-out's by-sequence non-transfer (§9.27/§9.31) shows up here as a thin 1.233-vs-1.501 margin rather than disappearing. Refuses to report if the targets do not move |
...
| `eye_confidence.py` | When it answers, is it right — and does abstaining reach 0.90? | the threshold is **chosen on one half of the sequences and reported on the other**, because the first version picked "the lowest threshold clearing 0.90" and quoted that same number — worth **+0.121** of pure optimism. The headline is a *curve* on held-out sequences that selects nothing; the select-then-report arm is kept beside it so the cost stays visible. Plus an **absent tag** — an object not in the footage — which passes the gate 0.000 of the time, so the confidence is evidence rather than a number |
...
| `brain_naming.py` | Does a real percept ground into a concept — and does *hearing* help *seeing*? | five categories each spread over **two or three different VOT2019 scenes**, without which "generalises to a new scene" cannot be asked at all; the decisive arm pairs each crop with a real ESC-50 clip drawn from a **random** concept, so sound is present, real and bound but uninformative, and the gap is what hearing contributes as a number rather than an intuition (**+0.282 [+0.103, +0.474]** within-scene); labels are permuted **five** times because one permutation is a draw from the null, not the null, and the verdict uses the worst draw. It also states two controls' limits rather than quoting them: the shuffled arm sits *below* chance because leave-one-sequence-out thins the held-out class, and it is no control at all for the sound→concept direction, since permuting labels permutes the paired clips with them. The eye is developed once on all scenes, so the cross-scene number is declared an **upper bound** — and it is chance anyway |

| `attend_canvas.py` | Is the canvas the limiting factor on top-down attention, as §9.32 concluded? | **two methods × two canvases**, which §9.32 could not do because it only ever had one canvas — so "tag type" and "map size" stop being the same variable; the image is identical across a cued pair and only the cue changes, making a feed-forward switch rate 0 by construction. It refuses to quote the archived 0.350 as its control, because **nothing in the tree calls `attend_spatial`** and that table is not reproducible — both canvases are re-measured in one run instead, and the ordering replicates while the magnitudes do not. Its decisive measurement is not the outcome but the mechanism: the fraction of top-down gain mass landing on the cued object's half of the canvas (chance 0.500) comes out **+0.032 at 56 px and +0.009 at 224 px**, so `attend` is not spatial selection at either size, and reported as margins rather than a `> 0.5` boolean that +0.009 would satisfy |

| `imagination_real.py` | Does the imagination system work on the real eye and real video — and *which part* of it? | five tests kept **separate**, because a single "imagination score" would have hidden that they disagree: imagery fails while sequences, rollout, surprise and cross-modal recall all pass. Each is built around the failure mode that mimics success — §9.19's novelty/coherence pair so novelty cannot be bought by collapsing coherence, a shuffled-**time** control with an identical marginal so only temporal structure is destroyed, persistence as the rollout arm to beat, and a KL split into **off-diagonal** transitions because the full matrix is dominated by "it stayed the same". The deciding control is three stored crops averaged with no concept layer at all, and it lands exactly on the target. Rollout skill is paired over the 13 videos, not pooled over ~400 correlated frames; it also records that its own verbatim rate of 0.000 is a cell-count artefact and not a repair of §9.19 |
